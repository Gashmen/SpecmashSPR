...
class ScaleBorder:
    ...
    def calculate_len0_x(self,list_cover):
        '''Принимает glands_on_sides_dict c dxf информацией'''
        rus_cover_name = 'Крышка'
        max_length_gland = 0
        if len(list_cover[rus_cover_name])>0:
            max_length_gland = max([gland.gland_length_dxf for gland in list_cover[rus_cover_name]])
    ...
    def calculate_len_x_bottom(self,len_x_bottom):
        needed_parametrs = [self.len0_x, self.len1_x,self.len2_x,self.len3_x,self.len4_x]
        sum_len_x_bottom = sum(needed_parametrs)
        if sum_len_x_bottom <= len_x_bottom:
            return True
        else:
            return False

    def calculate_len_x_top(self,len_x_top):
        needed_parametrs = [self.len0_x,self.len1_x,self.len2_x,self.len3_x,self.len4_x,
                            self.len5_x,self.len6_x,self.len7_x,self.len8_x,self.len9_x,
                            self.len10_x,self.len11_x]
        sum_len_x_top = sum(needed_parametrs)
        if sum_len_x_top <= len_x_top:
            return True
        else:
            return False

    def calculate_len_y_left(self,len_y_left):
        needed_parametrs = [self.len0_y,self.len1_y,self.len2_y,self.len3_y,self.len4_y,self.len5_y,self.len6_y]
        sum_len_y_left = sum(needed_parametrs)
        if sum_len_y_left <= len_y_left:
            return True
        else:
            return False

    def calculate_len_y_right(self,len_y_right):
        needed_parametrs = [self.len2_y,self.len3_y,self.len4_y,self.len5_y,self.len6_y]
        sum_len_y_left = sum(needed_parametrs)
        if sum_len_y_left <= len_y_right:
            return True
        else:
            return False
